...
uint32_t SerialKeyPressed(uint8_t *key)
{
  ...
  if ( USART_GetFlagStatus(DBG_COM1, USART_FLAG_RXNE) != RESET)
  {
    *key = (uint8_t)DBG_COM1->DR;
    return 1;
  }
  else
  {
...
{
  USART_SendData(DBG_COM1, c);
  while (USART_GetFlagStatus(DBG_COM1, USART_FLAG_TXE) == RESET)
  {
  }
}
